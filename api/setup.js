...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (parseInt(permitCheck.rows[0].count) === 0) {
      // Find actual karyawan IDs in the database to prevent foreign key violations
      const karyawanRes = await db.query("SELECT id FROM users WHERE role = 'karyawan' LIMIT 2");
      if (karyawanRes.rows.length > 0) {
        const id1 = karyawanRes.rows[0].id;
        const id2 = karyawanRes.rows[1] ? karyawanRes.rows[1].id : id1;
